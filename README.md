...
2. Did the simulation study (Time-Repeat = 1000, n_bootstrap = 1000)
result is inside manuscript
3. Fix the x generate value problem, now x is generated by randomly
pick 1000 values repeatly. x <- runif(1000, min = 0, max = 30) 

#### Question:

1. Actually I did three functions, y = 0.25*(x-0.9) returns good result,
but the other two has some problems in the middle...
...
First of all I define a function, let's say I define a function y = 0.25*(x-0.9)
-- which is the first function from the reference paper

To ensure shape restricted (monotone in this situation), we want to fit an i-spline
regression

Add some noise in the true function to simulate the dataset (I choose to add up
noise by standard normal mean = 0, sd = 0.1) -- noise <- rnorm(num_points, mean = 0, sd = 0.1)

Note: I generate dataset by x <- seq(0, 30, by = 0.1) which means 300 data for each 
dataset

Second I decide ten certain time points(let's say all the integers from 1 to 10)
as x and find out their y value

I want to add up some noise to those y values in certain time points, the noise
is also rnorm(num_points, mean = 0, sd = 0.1)

I do a bootstrap sampling for 1000 times to find out one 95% CI for each time
points

See wheather those CIs are true 95% CIs, we want to see that wheather 95% of
those data points we generate by add up the noise to the real value
are inside the CI.
